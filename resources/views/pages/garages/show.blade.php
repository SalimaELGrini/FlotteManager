...
<script src="https://unpkg.com/lucide@latest/dist/umd/lucide.js"></script>
    <title>{{ __('garage.title') }}</title>
</head>
<body>
<div class="container mt-5">
    <div class="card shadow-lg border-0 rounded-3">
        <div class="card-header text-white text-center" style="background-color: #0070BB;">
            <h3 class="mb-0">{{ __('garage.title') }}</h3>
        </div>
        <div class="card-body">
            <!-- Bouton PDF -->
            <div class="text-end mb-3">
                <a href="{{ route('garages.single.pdf', $garage->id) }}" class="btn btn-link-primary" style="color: #0070BB;" target="_blank">
                    <i data-lucide="file-down"></i> {{ __('garage.download_pdf') }}
                </a>
            </div>
        ...
        <div class="card-footer text-center">
            <a href="{{ route('garages.index') }}" class="btn btn-secondary btn-lg">{{ __('garage.back') }}</a>
        </div>
    </div>
</div>
